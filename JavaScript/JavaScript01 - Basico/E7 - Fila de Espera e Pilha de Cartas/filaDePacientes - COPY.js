...
//Criação do Array. Eu escolhi começar com alguns nomes, porém poderia ser um Arrau zerado. "let filaDePacientes = []"
let filaDePacientes = ["Maria", "Leonardo", "Mariane"];

do {
    let pacientes = "Estes são os pacientes esperando na fila:\n\n" //Necessário esta variável nessa posição, para que quando a operação for refeita pelo Do..While, "zerar" a string "pacientes"
    for (i = 0; i < filaDePacientes.length; i++) {
        pacientes += (i + 1) + " - " + filaDePacientes[i] + "\n" //Necessário o (i + 1) para que a listagem de paciente comece em 1 ao invés de 0
    }
